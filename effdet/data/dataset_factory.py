    ...
    if name.startswith("coco"):
        ...
        for s in splits:
            ...
            parser_cfg = CocoParserCfg(
                ann_filename=ann_file, has_labels=split_cfg["has_labels"]
            )
        ...
        for s in splits:
            ...
            if isinstance(split_cfg["split_filename"], (tuple, list)):
                assert len(split_cfg["split_filename"]) == len(
                    split_cfg["ann_filename"]
                )
                parser = None
                for sf, af, id in zip(
                    split_cfg["split_filename"],
                    split_cfg["ann_filename"],
                    split_cfg["img_dir"],
                ):
                ...
                parser_cfg = VocParserCfg(
                    ...
                    img_filename=os.path.join(
                        split_cfg["img_dir"], dataset_cfg.img_filename
                    ),
                )
                parser = create_parser(dataset_cfg.parser, cfg=parser_cfg)
        ...
        if "challenge2019" in name:
            ...
            dataset_cfg = OpenImagesObjV6SubsetCfg()
        else:
