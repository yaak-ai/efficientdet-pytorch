...
class OpenImagesSubsetParser(Parser):
    ...
    def _load_annotations(
        ...
        self.cat_id_to_label = {
            c: i + self.label_offset for i, c in enumerate(self.cat_ids)
        }
        ...
        def _load_img_info(csv_file, select_img_ids=None):
            ...
            img_info_df = img_info_df[
                (img_info_df["width"] >= self.min_img_size)
                & (img_info_df["height"] >= self.min_img_size)
            ]
        ...
        if self.include_masks and self.has_labels:
            ...
            img_sizes, img_id_to_idx = _load_img_info(
                img_info_filename, select_img_ids=anno_img_ids
            )
            ...
            self._anns = dict(
                bbox=masks_df[
                    ["BoxXMin", "BoxYMin", "BoxXMax", "BoxYMax"]
            ...
            bbox_df["LabelIdx"] = (
                bbox_df["LabelName"].map(self.cat_id_to_label).astype(np.int32)
            )

            self._anns = dict(
                bbox=bbox_df[["XMin", "YMin", "XMax", "YMax"]].values.astype(
                    np.float32
                ),
    ...
    def get_ann_info(self, idx):
        ...
        ann_values = tuple(
            self._anns[k][start_idx : start_idx + num_ann] for k in ann_keys
        )
    ...
    def _parse_ann_info(self, img_idx, ann_keys, ann_values):
        ...
        if self.include_bboxes_ignore:
            ann.update(
                dict(
                    bbox_ignore=gt_bboxes_ignore,
                    cls_ignore=np.array([], dtype=np.int64),
                )
            )
        if self.include_masks:
            ann["masks"] = gt_masks
