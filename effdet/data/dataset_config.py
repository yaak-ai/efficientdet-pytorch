...
class Coco2017Cfg(CocoCfg):
    variant: str = "2017"
    splits: Dict[str, dict] = field(
        default_factory=lambda: dict(
            train=dict(
                ann_filename="annotations/instances_train2017.json",
                img_dir="train2017",
                has_labels=True,
            ),
            val=dict(
                ann_filename="annotations/instances_val2017.json",
                img_dir="val2017",
                has_labels=True,
            ),
            test=dict(
                ann_filename="annotations/image_info_test2017.json",
                img_dir="test2017",
                has_labels=False,
            ),
            testdev=dict(
                ann_filename="annotations/image_info_test-dev2017.json",
                img_dir="test2017",
                has_labels=False,
            ),
        )
...
class Coco2014Cfg(CocoCfg):
    ...
    splits: Dict[str, dict] = field(
        default_factory=lambda: dict(
            train=dict(
                ann_filename="annotations/instances_train2014.json",
                img_dir="train2014",
                has_labels=True,
            ),
            val=dict(
                ann_filename="annotations/instances_val2014.json",
                img_dir="val2014",
                has_labels=True,
            ),
...
class OpenImagesObjSubsetCfg(OpenImagesCfg):
    num_classes: int = 2
    categories_map: str = "annotations/class-descriptions-boxable-subset.csv"
...
class OpenImagesObjV5Cfg(OpenImagesObjCfg):
    splits: Dict[str, dict] = field(
        default_factory=lambda: dict(
            train=dict(
                img_dir="train",
                img_info="annotations/train-info.csv",
                has_labels=True,
                prefix_levels=1,
            ...
            val=dict(
                img_dir="validation",
                img_info="annotations/validation-info.csv",
                has_labels=True,
                prefix_levels=0,
...
@dataclass
class OpenImagesObjV6SubsetCfg(OpenImagesObjSubsetCfg):
    splits: Dict[str, dict] = field(
        default_factory=lambda: dict(
            train=dict(
                img_dir="train",
...
class OpenImagesObjChallenge2019Cfg(OpenImagesObjCfg):
    num_classes: int = 500
    categories_map: str = (
        "annotations/challenge-2019/challenge-2019-classes-description-500.csv"
    )
    splits: Dict[str, dict] = field(
        default_factory=lambda: dict(
            train=dict(
            ...
            test=dict(
                img_dir="challenge2019",
                img_info="annotations/challenge-2019/challenge2019-info",
                prefix_levels=0,
                has_labels=False,
                ann_bbox="",
                ann_img_label="",
...
class OpenImagesSegV5Cfg(OpenImagesSegCfg):
    num_classes: int = 300
    splits: Dict[str, dict] = field(
        default_factory=lambda: dict(train=dict(), val=dict(), test=dict())
    )
...
class OpenImagesSegChallenge2019Cfg(OpenImagesSegCfg):
    num_classes: int = 300
    ann_class_map: str = (
        "annotations/challenge-2019/challenge-2019-classes-description-segmentable.csv"
    )
    splits: Dict[str, dict] = field(
        default_factory=lambda: dict(train=dict(), val=dict(), test=dict())
    )
